...
exports.WalletAdvertiser = void 0;
...
class WalletAdvertiser {
     ...
     */
    constructor(chain, privateKey, storageURL, advertisableURI) {
        this.chain = chain;
        this.privateKey = privateKey;
        this.storageURL = storageURL;
        this.advertisableURI = advertisableURI;
        if (!(0, isAdvertisableURI_js_1.isAdvertisableURI)(advertisableURI)) {
     ...
     */
    async initWithEngine(engine) {
        const client = new wallet_toolbox_client_1.StorageClient(this.wallet, this.storageURL);
    ...
    async createAdvertisements(adsData) {
        ...
        const outputs = await Promise.all(adsData.map(async (ad) => {
            if (!(0, isValidTopicOrServiceName_js_1.isValidTopicOrServiceName)(ad.topicOrServiceName)) {
            ...
            const lockingScript = await pushdrop.lock([
                sdk_1.Utils.toArray(ad.protocol, 'utf8'),
                sdk_1.Utils.toArray(identityKey, 'hex'),
                sdk_1.Utils.toArray(this.advertisableURI, 'utf8'),
                sdk_1.Utils.toArray(ad.topicOrServiceName, 'utf8')
            ], [2, ad.protocol === 'SHIP' ? 'service host interconnect' : 'service lookup availability'], '1', 'anyone', true);
            return {
        ...
        const beef = sdk_1.Transaction.fromAtomicBEEF(tx.tx).toBEEF();
        return {
            beef,
    ...
    async revokeAdvertisements(advertisements) {
        ...
        const signableTx = sdk_1.Transaction.fromAtomicBEEF(revokePartial.signableTransaction.tx);
        const spends = {};
        const pushdrop = new sdk_1.PushDrop(this.wallet);
        // Sign the inputs now that wwe have the transaction
        for (let i = 0; i < advertisements.length; i++) {
            const advertisement = advertisements[i];
            const unlocker = pushdrop.unlock([2, advertisement.protocol === 'SHIP' ? 'service host interconnect' : 'service lookup availability'], '1', 'self');
        ...
        return {
            beef: sdk_1.Transaction.fromAtomicBEEF(revokeTx.tx).toBEEF(),
    ...
    parseAdvertisement(outputScript) {
        try {
            ...
            const topicOrService = sdk_1.Utils.toUTF8(result.fields[3]);
...
exports.WalletAdvertiser = WalletAdvertiser;
//# sourceMappingURL=WalletAdvertiser.js.map