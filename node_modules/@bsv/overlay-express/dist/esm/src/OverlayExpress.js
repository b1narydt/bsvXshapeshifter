...
import { ARC, MerklePath, WhatsOnChain, HTTPSOverlayBroadcastFacilitator, DEFAULT_TESTNET_SLAP_TRACKERS } from '@bsv/sdk';
...
export default class OverlayExpress {
     ...
     */
    configureChainTracker(chainTracker = new WhatsOnChain(this.network)) {
    ...
    async configureEngine(autoConfigureShipSlap = true) {
        ...
        this.engine = new Engine(this.managers, this.services, storage, 
        // chainTracker
    ...
    async start() {
        ...
        if (this.verboseRequestLogging) {
            this.app.use((req, res, next) => {
                ...
                res.on('finish', () => {
                    ...
                    this.logger.log(chalk.magenta.bold(`📤 Outgoing Response: ${req.method} ${req.originalUrl} - Status: ${res.statusCode} - Duration: ${duration}ms`));
        ...
        if (this.engine?.advertiser instanceof DiscoveryServices.WalletAdvertiser) {
            this.logger.log(chalk.cyan(`${this.name} will now advertise with SHIP and SLAP as appropriate at FQDN: ${this.advertisableFQDN}`));
