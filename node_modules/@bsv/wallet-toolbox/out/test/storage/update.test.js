...
describe('update tests', () => {
    ...
    const chain = 'test';
    const env = TestUtilsWalletStorage_1._tu.getEnvFlags(chain);
    ...
    beforeEach(async () => {
        ...
        const localSQLiteFile = await TestUtilsWalletStorage_1._tu.newTmpFile(`${databaseName}.sqlite`, false, false, true);
        const knexSQLite = TestUtilsWalletStorage_1._tu.createLocalSQLite(localSQLiteFile);
        storages.push(new index_all_1.StorageKnex({
            ...index_all_1.StorageKnex.defaultOptions(),
            chain,
            knex: knexSQLite
        }));
        if (env.runMySQL) {
            const knexMySQL = TestUtilsWalletStorage_1._tu.createLocalMySQL(`${databaseName}.mysql`);
            storages.push(new index_all_1.StorageKnex({
                ...index_all_1.StorageKnex.defaultOptions(),
                chain,
                knex: knexMySQL
    ...
    test('0_update ProvenTx', async () => {
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                const t = (0, index_all_1.verifyOne)(await storage.findProvenTxs({
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedTx = (0, index_all_1.verifyOne)(await storage.findProvenTxs({
                    ...
                    for (const [key, value] of Object.entries(testValues)) {
                        ...
                        if (typeof value === 'string') {
                            ...
                            expect(r1).toBe(1);
                            const updatedRow = (0, index_all_1.verifyOne)(await storage.findProvenTxs({
                                partial: { [primaryKey]: record[primaryKey] }
                            }));
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    expect(r1).toBe(1);
                    const updatedRow = (0, index_all_1.verifyOne)(await storage.findProvenTxReqs({
                        partial: { [primaryKey]: record[primaryKey] }
                    }));
                    for (const [key, value] of Object.entries(testValues)) {
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(await storage.findUsers({
                        partial: { [primaryKey]: record[primaryKey] }
                    }));
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    expect(r1).toBe(1);
                    const updatedRow = (0, index_all_1.verifyOne)(await storage.findCertificates({
                        partial: { [primaryKey]: record[primaryKey] }
                    }));
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated CertificateField with certificateId=${record.certificateId}, fieldName=${testValues.fieldName} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated OutputBasket with basketId=${record.basketId}, name=${testValues.name} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        if (typeof actualValue === 'boolean') {
                            if (value === 1) {
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated Transaction with transactionId=${record.transactionId} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
    ...
    test('7a updateTransactionStatus', async () => {
        ...
        let tx = (0, index_all_1.verifyOne)(await storage.findTransactions({
            partial: { status: 'unproven' },
            paged: { limit: 1 }
        }));
        expect(tx.status).toBe('unproven');
        await storage.updateTransactionStatus('completed', tx.transactionId);
        tx = (0, index_all_1.verifyOne)(await storage.findTransactions({
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated Commission with commissionId=${record.commissionId} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated Output with outputId=${record.outputId} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated OutputTag with outputTagId=${record.outputTagId} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        if (typeof actualValue === 'boolean') {
                            if (value === 1) {
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated OutputTagMap with outputTagId=${record.outputTagId} and outputId=${record.outputId} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        if (typeof actualValue === 'boolean') {
                            if (value === 1) {
    ...
    test('12_update TxLabel', async () => {
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated TxLabel with txLabelId=${record.txLabelId} was not unique or missing.`);
                for (const [key, value] of Object.entries(testValues)) {
                    const actualValue = updatedRow[key];
                    if (typeof actualValue === 'boolean') {
                        if (value === 1) {
        ...
        for (const { storage, setup } of setups) {
            ...
            for (const record of records) {
                ...
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated TxLabelMap with transactionId=${record[primaryKeyTransaction]} and txLabelId=${record[primaryKeyLabel]} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
        ...
        for (const { storage, setup } of setups) {
            ...
            for (const record of records) {
                try {
                    ...
                    const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated MonitorEvent with id=${record.id} was not unique or missing.`);
                    for (const [key, value] of Object.entries(testValues)) {
                        const actualValue = updatedRow[key];
                        const normalizedActual = (0, TestUtilsWalletStorage_2.normalizeDate)(actualValue);
                        const normalizedExpected = (0, TestUtilsWalletStorage_2.normalizeDate)(value);
    ...
    test('15_update SyncState', async () => {
        ...
        for (const { storage } of setups) {
            ...
            for (const record of records) {
                ...
                const updatedRow = (0, index_all_1.verifyOne)(updatedRecords, `Updated SyncState with syncStateId=${record.syncStateId} was not unique or missing.`);
                for (const [key, value] of Object.entries(testValues)) {
                    const actualValue = updatedRow[key];
                    if (typeof actualValue === 'boolean') {
                        if (value === 1) {
