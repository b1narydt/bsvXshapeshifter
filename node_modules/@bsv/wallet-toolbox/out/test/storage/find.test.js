...
describe('find tests', () => {
    ...
    beforeAll(async () => {
        const localSQLiteFile = await TestUtilsWalletStorage_1._tu.newTmpFile('storagefindtest.sqlite', false, false, true);
        ...
        if (env.runMySQL) {
            const knexMySQL = TestUtilsWalletStorage_1._tu.createLocalMySQL('storagefindtest');
            storages.push(new StorageKnex_1.StorageKnex({
                ...StorageKnex_1.StorageKnex.defaultOptions(),
                chain,
                knex: knexMySQL
    ...
    test('3 find Certificate', async () => {
        for (const { storage, setup } of setups) {
            ...
            expect((await storage.findCertificates({
                partial: {},
    ...
    test('4 find CertificateField', async () => {
        for (const { storage, setup } of setups) {
            ...
            expect((await storage.findCertificateFields({
    ...
    test('5 find OutputBasket', async () => {
        for (const { storage, setup } of setups) {
            ...
            expect((await storage.findOutputBaskets({
                partial: {},
