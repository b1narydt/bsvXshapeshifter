...
class TestUtilsWalletStorage {
    ...
    static noEnv(chain) {
        try {
            index_all_1.Setup.getEnv(chain);
            return false;
        }
        catch (_a) {
            return true;
    ...
    static async createWalletOnly(args) {
        ...
        const serviceOptions = index_all_1.Services.createDefaultOptions(env.chain);
        ...
        if (args.privKeyHex) {
            ...
            privilegedKeyManager = new index_all_1.sdk.PrivilegedKeyManager(async () => privKey);
        }
    ...
    static async createTestWallet(args) {
        ...
        if (typeof args === 'string') {
            ...
            if (!env.testIdentityKey || !env.testFilePath) {
                throw new index_all_1.sdk.WERR_INVALID_PARAMETER('env.testIdentityKey and env.testFilePath', 'valid');
            }
        ...
        if (useMySQLConnectionForClient) {
            ...
            if (!env.cloudMySQLConnection)
                throw new index_all_1.sdk.WERR_INVALID_PARAMETER('env.cloundMySQLConnection', 'valid');
            const connection = JSON.parse(env.cloudMySQLConnection);
            client = new index_all_1.StorageKnex({
            ...
            client = new index_all_1.StorageClient(setup.wallet, endpointUrl);
        }
        ...
        if (addLocalBackup) {
            ...
            const localBackup = new index_all_1.StorageKnex({
                ...index_all_1.StorageKnex.defaultOptions(),
                knex: _tu.createLocalSQLite(backupPath),
                chain
            });
            await localBackup.migrate(backupName, (0, index_all_1.randomBytesHex)(33));
        ...
        if (setup.storage.getActiveStore() === setup.localStorageIdentityKey) {
            const basket = (0, index_all_1.verifyOne)(await setup.activeStorage.findOutputBaskets({
                partial: {
        ...
        if (balance.total < 1000) {
            throw new index_all_1.sdk.WERR_INSUFFICIENT_FUNDS(1000, 1000 - balance.total);
        }
    ...
    static async createKnexTestWalletWithSetup(args) {
        ...
        await activeStorage.migrate(args.databaseName, (0, index_all_1.randomBytesHex)(33));
    ...
    static async createLegacyWalletCopy(databaseName, walletKnex, tryCopyToPath) {
        ...
        if (useReader)
            await activeStorage.dropAllData();
        await activeStorage.migrate(databaseName, (0, index_all_1.randomBytesHex)(33));
        await activeStorage.makeAvailable();
    ...
    static async insertTestOutputBasket(storage, u, partial) {
        ...
        if (u === undefined) {
            ...
            user = (0, index_all_1.verifyOne)(await storage.findUsers({ partial: { userId: u } }));
        }
        else {
            user = u;
        }
    ...
    static async createWalletSetupEnv(chain) {
        const env = index_all_1.Setup.getEnv(chain);
    ...
    static async createNoSendTxPair(wallet, satoshis = 41) {
        ...
        const derivationPrefix = (0, index_all_1.randomBytesBase64)(8);
        const derivationSuffix = (0, index_all_1.randomBytesBase64)(8);
