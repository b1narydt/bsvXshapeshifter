...
describe('ARC tests', () => {
    ...
    const arcTest = new ARC_1.ARC((0, createDefaultWalletServicesOptions_1.arcDefaultUrl)(envTest.chain), {
        apiKey: envTest.taalApiKey
    });
    const envMain = TestUtilsWalletStorage_1._tu.getEnv('main');
    const arcMain = new ARC_1.ARC((0, createDefaultWalletServicesOptions_1.arcDefaultUrl)(envMain.chain), {
        apiKey: envMain.taalApiKey
    });
    test.skip('0 double spend', async () => {
        const arc = arcTest;
        const beef = sdk_1.Beef.fromString(testnetDoubleSpendBeef);
...
async function postBeefTest(chain, arc) {
...
async function postRawTxTest(chain, arc) {
    if (index_all_1.Setup.noEnv(chain))
        return;
    const c = await TestUtilsWalletStorage_1._tu.createNoSendTxPair(chain);
