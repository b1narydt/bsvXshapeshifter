...
describe('whatsonchain tests', () => {
    ...
    if (TestUtilsWalletStorage_1._tu.noTestEnv('test'))
        return;
    const envTest = TestUtilsWalletStorage_1._tu.getEnv('test');
    const wocTest = new WhatsOnChain_1.WhatsOnChain(envTest.chain, {
        apiKey: envTest.taalApiKey
    });
    const envMain = TestUtilsWalletStorage_1._tu.getEnv('main');
    const wocMain = new WhatsOnChain_1.WhatsOnChain(envMain.chain, {
    ...
    test('3 getMerklePath mainnet', async () => {
        const services = new Services_1.Services(envMain.chain);
        {
    ...
    test.skip('7 postRawTx testnet', async () => {
        if (index_all_1.Setup.noEnv('test'))
            return;
        const woc = wocTest;
        const c = await TestUtilsWalletStorage_1._tu.createNoSendTxPair('test');
    ...
    test.skip('7a nosend cleanup testnet', async () => {
        const c = await TestUtilsWalletStorage_1._tu.createWalletSetupEnv('test');
    ...
    test.skip('8 postRawTx mainnet', async () => {
        if (index_all_1.Setup.noEnv('main'))
            return;
        const woc = wocMain;
        const c = await TestUtilsWalletStorage_1._tu.createNoSendTxPair('main');
        const rawTxDo = c.beef.findTxid(c.txidDo).tx.toHex();
        const rawTxUndo = c.beef.findTxid(c.txidUndo).tx.toHex();
        const txidDo = await woc.postRawTx(rawTxDo);
        expect(txidDo).toBe(c.txidDo);
    ...
    test.skip('8b run monitor mainnet', async () => {
        if (index_all_1.Setup.noEnv('main'))
            return;
        if (!index_all_1.Setup.getEnv('main').filePath)
            return;
    ...
    test.skip('8c backup to cloud', async () => {
        ...
        if (!index_all_1.Setup.getEnv('main').filePath)
            return;
        const c = await TestUtilsWalletStorage_1._tu.createWalletSetupEnv('main');
        const client = new index_client_1.StorageClient(c.wallet, 'https://storage.babbage.systems');
        await c.storage.addWalletStorageProvider(client);
        await c.storage.updateBackups();
        await c.wallet.destroy();
    });
});
//# sourceMappingURL=WhatsOnChain.test.js.map