...
const Commission_1 = require("../../../../../src/storage/schema/entities/Commission");
describe('Commission class method tests', () => {
    ...
    beforeAll(async () => {
        if (env.runMySQL) {
            ctxs.push(await TestUtilsWalletStorage_1.TestUtilsWalletStorage.createLegacyWalletMySQLCopy('CommissionTests'));
            ctxs2.push(await TestUtilsWalletStorage_1.TestUtilsWalletStorage.createLegacyWalletMySQLCopy('CommissionTests2'));
        }
        ctxs.push(await TestUtilsWalletStorage_1.TestUtilsWalletStorage.createLegacyWalletSQLiteCopy('CommissionTests'));
        ctxs2.push(await TestUtilsWalletStorage_1.TestUtilsWalletStorage.createLegacyWalletSQLiteCopy('CommissionTests2'));
    });
    afterAll(async () => {
    ...
    test('0_equals identifies matching Commission entities', async () => {
        for (const { activeStorage } of ctxs) {
            ...
            const entity1 = new Commission_1.EntityCommission(initialData);
            const entity2 = new Commission_1.EntityCommission(initialData);
    ...
    test('1_equals identifies non-matching Commission entities', async () => {
        for (const { activeStorage } of ctxs) {
            ...
            for (const mismatch of mismatchedEntities) {
                const mismatchedEntity = new Commission_1.EntityCommission({
                    ...initialData,
                    ...mismatch
                });
    ...
    test('2_mergeExisting updates entity and database when ei.updated_at > this.updated_at', async () => {
        for (const { activeStorage } of ctxs) {
            ...
            await activeStorage.insertCommission(initialData);
            // Create a Commission entity from the initial data
            const entity = new Commission_1.EntityCommission(initialData);
    ...
    test('3_mergeExisting does not update when ei.updated_at <= this.updated_at', async () => {
        for (const { activeStorage } of ctxs) {
            ...
            syncMap.transaction.idMap[transactionId] = transactionId;
            // Call mergeExisting
            const wasMergedRaw = await entity.mergeExisting(activeStorage, undefined, olderOrEqualData, syncMap, undefined // `trx` is not used
            );
            const wasMerged = Boolean(wasMergedRaw);
...
//# sourceMappingURL=CommissionTests.test.js.map