...
class StorageReaderWriter extends StorageReader_1.StorageReader {
    ...
    async setActive(auth, newActiveStorageIdentityKey) {
        return await this.updateUser((0, index_client_1.verifyId)(auth.userId), {
    ...
    async findTransactionById(id, trx, noRawTx) {
        return (0, index_client_1.verifyOneOrNone)(await this.findTransactions({
    ...
    async findOrInsertOutputBasket(userId, name, trx) {
        ...
        for (let retry = 0;; retry++) {
            try {
                ...
                if (basket.isDeleted) {
                    await this.updateOutputBasket((0, index_client_1.verifyId)(basket.basketId), {
                        isDeleted: false
                    });
                }
                return basket;
        ...
        for (let retry = 0;; retry++) {
            try {
                ...
                if (txLabel.isDeleted) {
                    await this.updateTxLabel((0, index_client_1.verifyId)(txLabel.txLabelId), {
                        isDeleted: false
                    });
                }
                return txLabel;
        ...
        for (let retry = 0;; retry++) {
            try {
                ...
                if (outputTag.isDeleted) {
                    await this.updateOutputTag((0, index_client_1.verifyId)(outputTag.outputTagId), {
                        isDeleted: false
                    });
                }
                return outputTag;
