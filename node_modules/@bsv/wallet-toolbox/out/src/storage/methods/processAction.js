...
async function shareReqsWithWorld(storage, userId, txids, isDelayed) {
    ...
    const readyToSendReqs = r.details.filter(d => d.status === 'readyToSend').map(d => new index_client_1.EntityProvenTxReq(d.req));
    ...
    for (const ar of ars) {
        ...
        if (d) {
            ...
            if (d.pbrft.status === 'error' && ar.getReq.req) {
                ...
                const req = await index_client_1.EntityProvenTxReq.fromStorageId(storage, ar.getReq.req.provenTxReqId);
