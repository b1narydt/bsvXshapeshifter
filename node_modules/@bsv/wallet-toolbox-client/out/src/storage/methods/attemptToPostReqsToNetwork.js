...
async function attemptToPostReqsToNetwork(storage, reqs, trx) {
    ...
    for (const rb of reqs) {
        ...
        if (!badReq) {
            try {
                ...
                if (e.code === 'WERR_INVALID_PARAMETER' && e.parameter === 'txid') {
                    badReq = true;
    ...
    if (!r.pbr) {
        ...
        for (const d of r.details) {
            ...
            if (!pbrft)
                throw new index_client_1.sdk.WERR_INTERNAL(`postBeef service failed to return result for txid ${d.txid}`);
            d.pbrft = pbrft;
            if (r.pbr.data)
    ...
    for (const d of r.details) {
        ...
        // 3. unknown: req status from unprocessed to sending or remains sending, tx remains sending
            ...
            throw new index_client_1.sdk.WERR_INTERNAL(`unexpected status ${d.status}`);
        }
        if (newReqStatus) {
        ...
        if (newTxStatus) {
            ...
            if (!ids || ids.length < 1)
                throw new index_client_1.sdk.WERR_INTERNAL(`req must have at least one transactionId to notify`);
            for (const id of ids) {
